/**
 * ============================================================
 * JDI 脈動傳媒 · 主播適配度心理測驗（專業版 60 題）· 資料層
 *
 * 用途：主播招募初篩、培訓評估、自我測試、經紀公司 / MCN 分流
 *
 * 檔案負責：
 *   - 6 大模組定義 (STMT_MODULES)
 *   - 60 題主題庫 (STMT_QUESTIONS)
 *   - 3 題誠實作答檢核題 (STMT_LIE_QUESTIONS) — 不列入主分
 *   - 5 種風險規則 (STMT_RISK_RULES)
 *   - 6 種主播分型定義 + 判定規則 (STMT_PROFILES, STMT_PROFILE_RULES)
 *   - 總分區間分級 (STMT_TIERS)
 *   - 依據分型 / 分數 / 風險組合結果文案的模板 (STMT_ADVICE_TEMPLATES)
 *
 * 雙 runtime 相容：
 *   - 瀏覽器：所有 STMT_* 掛到 window.*
 *   - Node.js：module.exports 匯出（給 API handler / test 使用）
 *
 * 反向題資訊直接內嵌在每題的 reverse 欄位，避免與外部 ID 陣列不同步。
 * ============================================================
 */
...
/* ============================================================
 *  型別（JSDoc @typedef — 供 IDE / 維護者參考）
 * ============================================================
 *
 * @typedef {'camera_expression'|'audience_interaction'|'emotional_regulation'|'self_discipline'|'content_creativity'|'boundary_control'} ModuleKey
 *
 * @typedef {Object} Question
 * @property {number} id            從 1 開始，遞增，唯一
 * @property {ModuleKey} module     所屬模組
 * @property {string} text          題目文字（zh-TW）
 * @property {boolean} reverse      是否反向計分
 *
 * @typedef {Object} LieQuestion
 * @property {number} id            L1 / L2 / L3 (以字串形式的 id)
 * @property {string} text          題目文字
 * @property {number} maxScore      閾值：>= 這個值視為過度理想化
 *
 * @typedef {Object.<string, number>} AnswerMap
 *   鍵為 String(id)，值為 1..5（原始分，不做反向轉換）
 *
 * @typedef {Object} ModuleScore
 * @property {ModuleKey} key
 * @property {number} raw           原始分 (10..50，反向已轉換)
 * @property {number} percent       百分制 (0..100)，一位小數
 *
 * @typedef {'high_exposure_anxiety'|'emotional_burst'|'three_minute_heat'|'content_drought'|'over_exposure'} RiskFlagKey
 *
 * @typedef {Object} RiskFlag
 * @property {RiskFlagKey} key
 * @property {string} name
 * @property {string} description
 * @property {string} advice
 *
 * @typedef {'stage'|'companion'|'knowledge'|'stable'|'high_potential'|'not_recommended'} ProfileTypeKey
 *
 * @typedef {'excellent'|'developing'|'potential'|'training'|'unstable'} TierKey
 *
 * @typedef {Object} TestResult
 * @property {number} totalScore              加權總分 (0..100)
 * @property {TierKey} tier                   總分區間
 * @property {ModuleScore[]} moduleScores     6 個模組分數（依 STMT_MODULES 順序）
 * @property {ProfileTypeKey} profileKey      主播分型
 * @property {RiskFlag[]} risks               命中的風險
 * @property {string} summary                 一段總結評語
 * @property {string[]} strengths             3 個核心優勢
 * @property {string[]} weaknesses            2 個目前短板
 * @property {string[]} recommendedPaths      適合的發展方向
 * @property {string[]} avoidPaths            不建議優先嘗試的路線
 * @property {string[]} actionItems           3 條可執行提升建議
 * @property {boolean} lieFlagged             誠實檢核異常
 * @property {number} version                 題庫版本
 * @property {string} completedAt             ISO 時間戳
 */
 ...
 * ============================================================ */
const STMT_DATA = {
  version: STMT_VERSION,
  modules: STMT_MODULES,
  questions: STMT_QUESTIONS,
  lieQuestions: STMT_LIE_QUESTIONS,
  lieThresholdAvg: STMT_LIE_THRESHOLD_AVG,
  reverseIds: STMT_REVERSE_IDS,
  riskRules: STMT_RISK_RULES,
  profiles: STMT_PROFILES,
  profileRules: STMT_PROFILE_RULES,
  profileFallback: STMT_PROFILE_FALLBACK,
  profilePaths: STMT_PROFILE_PATHS,
  tiers: STMT_TIERS,
  moduleAdvice: STMT_MODULE_ADVICE,
};

// 瀏覽器
if (typeof window !== 'undefined') {
  window.STMT_DATA = STMT_DATA;
  // 為方便個別存取，也逐項掛到 window
  window.STMT_VERSION = STMT_VERSION;
  window.STMT_MODULES = STMT_MODULES;
  window.STMT_QUESTIONS = STMT_QUESTIONS;
  window.STMT_LIE_QUESTIONS = STMT_LIE_QUESTIONS;
  window.STMT_RISK_RULES = STMT_RISK_RULES;
  window.STMT_PROFILES = STMT_PROFILES;
  window.STMT_PROFILE_RULES = STMT_PROFILE_RULES;
  window.STMT_PROFILE_PATHS = STMT_PROFILE_PATHS;
  window.STMT_TIERS = STMT_TIERS;
  window.STMT_MODULE_ADVICE = STMT_MODULE_ADVICE;
}

// Node.js（給 API handler / 測試用）
if (typeof module !== 'undefined' && module.exports) {
  module.exports = STMT_DATA;
}
