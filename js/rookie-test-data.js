/**
 * ============================================================
 * JDI 脈動傳媒 · 素人主播適配度心理測驗（1 分鐘快篩版 24 題）· 資料層
 *
 * 定位：這是「新素人版」測驗，不同於 60 題的 5min 深度版。
 *   - 目標受眾：完全沒有直播經驗、想快速了解自己適不適合當主播的素人
 *   - 24 題 · 6 模組（每模組 4 題）· 4 種分型 · 6 種風險 flag · 4 tier
 *   - 完成後給完整版報告（雷達圖 + 分析）
 *
 * 檔案負責：
 *   - 版本 / 模組定義（ROOK_MODULES）
 *   - 24 題主題庫（ROOK_QUESTIONS）
 *   - 反向題 ID（ROOK_REVERSE_IDS）
 *   - 6 種風險規則（ROOK_RISK_RULES）
 *   - 4 種素人分型定義 + 判定規則（ROOK_PROFILES, ROOK_PROFILE_RULES）
 *   - 總分區間分級（ROOK_TIERS）
 *   - 結果文案模板（moduleAdvice / profilePaths）
 *
 * 雙 runtime 相容：
 *   - 瀏覽器：所有 ROOK_* 掛到 window.*
 *   - Node.js：module.exports 匯出
 *
 * ⚠️ 這個檔案的內容應該與 /functions/api/_rookie-test-data.js 保持同步。
 *    (兩邊分別為 CommonJS/browser 版 vs ES Module 版；未來可做 build step。)
 * ============================================================
 */
...
/* ============================================================
 *  型別（JSDoc — 供 IDE / 維護者參考）
 * ============================================================
 *
 * @typedef {'expression'|'interaction'|'stability'|'discipline'|'contentPotential'|'boundary'} RookModuleKey
 *
 * @typedef {'natural_camera'|'content_sharer'|'companion_interactive'|'potential_rookie'} RookProfileKey
 *
 * @typedef {'stage_fright'|'passive_interaction'|'emotion_volatile'|'discipline_weak'|'topic_barren'|'boundary_blur'} RookRiskKey
 *
 * @typedef {'excellent'|'developing'|'potential'|'needs_training'} RookTierKey
 */
...
/* ============================================================
 *  模組定義（固定順序 = 顯示順序）
 *  素人版模組比 60 題深度版更聚焦「初階入門判斷」，key 名不同、加權也不同。
 * ============================================================ */
const ROOK_MODULES = [
  {
    key: 'expression',
...
/* ============================================================
 *  題庫（24 題 · 每模組 4 題 · reverse 對應題號）
 *
 *  反向題 ID：{ 2, 4, 8, 10, 12, 14, 19, 22, 24 }
 *  （9 題反向 = 每個模組 1-2 題反向，避免思考慣性）
 *
 *  作答量表：1 = 完全不同意 → 5 = 完全同意
 * ============================================================ */
...
/* ============================================================
 *  風險規則（模組分數 < 55 觸發）
 *  素人版有 6 種風險 flag，覆蓋 6 個模組各一種常見風險。
 * ============================================================ */
 ...
 *
 *  判定邏輯（依需求規格）：
 *    1) natural_camera         — 天生鏡頭感型（最嚴，先判）
 *    2) content_sharer         — 內容分享型
 *    3) companion_interactive  — 陪伴互動型
 *    4) potential_rookie       — 潛力待培養型（fallback，永遠會 match）
 * ============================================================ */
const ROOK_PROFILE_RULES = [
  ...
  // Rule 4：Fallback — 潛力待培養型
  // 只要有基本分（>= 40）就 match，否則走保底也是這個
  {
...
const ROOK_DATA = {
  ...
  modules: ROOK_MODULES,
  questions: ROOK_QUESTIONS,
  reverseIds: ROOK_REVERSE_IDS,
  riskRules: ROOK_RISK_RULES,
  profiles: ROOK_PROFILES,
  profileRules: ROOK_PROFILE_RULES,
  profileFallback: ROOK_PROFILE_FALLBACK,
  profilePaths: ROOK_PROFILE_PATHS,
  tiers: ROOK_TIERS,
  moduleAdvice: ROOK_MODULE_ADVICE,
};

// 瀏覽器
if (typeof window !== 'undefined') {
  window.ROOK_DATA = ROOK_DATA;
  window.ROOK_VERSION = ROOK_VERSION;
  window.ROOK_MODULES = ROOK_MODULES;
  window.ROOK_QUESTIONS = ROOK_QUESTIONS;
  window.ROOK_RISK_RULES = ROOK_RISK_RULES;
  window.ROOK_PROFILES = ROOK_PROFILES;
  window.ROOK_PROFILE_RULES = ROOK_PROFILE_RULES;
  window.ROOK_PROFILE_PATHS = ROOK_PROFILE_PATHS;
  window.ROOK_TIERS = ROOK_TIERS;
  window.ROOK_MODULE_ADVICE = ROOK_MODULE_ADVICE;
}

// Node.js（給測試 / 舊 CommonJS 引用）
if (typeof module !== 'undefined' && module.exports) {
  module.exports = ROOK_DATA;
}
